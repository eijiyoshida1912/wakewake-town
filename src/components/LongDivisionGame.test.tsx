...
describe('LongDivisionGame: 補助あり（かんたん・まあまあ）', () => {
  ...
  it('完成後に「つぎのおねがいへ」を押すと、onComplete が1回呼ばれる', () => {
    ...
    render(<LongDivisionGame problem={easyProblem} onComplete={onComplete} />)
    submit('3')
    submit('9')
    submit('0')
    drop(6)
...
describe('LongDivisionGame: 補助なし（チャレンジ）', () => {
  it('ひっ算の盤面に直接入力する形式で、質問文・ヒント・「こたえる！」は出ない', () => {
    render(<LongDivisionGame problem={challengeProblem} onComplete={() => {}} />)
  ...
  it('すべてのマスを正しく埋めると、「64 あまり 3」と ＋30コイン が表示される', () => {
    const onComplete = vi.fn()
    render(<LongDivisionGame problem={challengeProblem} onComplete={onComplete} />)
